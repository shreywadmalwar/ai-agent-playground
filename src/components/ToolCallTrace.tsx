...
export function ToolCallTrace({ toolCalls }: { toolCalls: ToolCall[] }) {
  ...
  return (
    <details
      className="group rounded-lg border border-amber-300 bg-amber-50 text-sm dark:border-zinc-700 dark:bg-zinc-900/60"
      open={false}
    >
      <summary className="cursor-pointer select-none px-3 py-2 font-medium text-amber-700 hover:text-amber-800 dark:text-amber-300/90 dark:hover:text-amber-200">
      ...
      <ol className="space-y-2 px-3 pb-3">
        {toolCalls.map((call, i) => (
          <li
            key={call.id + i}
            className="rounded-md border border-zinc-300 bg-white p-2 dark:border-zinc-800 dark:bg-zinc-950/70"
          >
            ...
            <div className="grid gap-1">
              <div>
                ...
                <code className="whitespace-pre-wrap break-all text-sky-700 dark:text-sky-300">
                  {pretty(call.input)}
                </code>
              </div>
              <div>
                <span className="text-zinc-500">output → </span>
                <code className="whitespace-pre-wrap break-all text-emerald-700 dark:text-emerald-300">
                  {pretty(call.output)}
                </code>
              </div>
            </div>
